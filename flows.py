...
# set to True to check on NaN/inf values during the foward propagation
# and check on extremely large values (>tresh)
global print_stuff
global tresh
print_stuff = False
thresh = 0.5
...
def print_errors(tensor, tensor_name):
    """
    Prints out stuff for debugging the flow.
    """
    mask_big = (tensor>thresh)
    mask_nan = (tensor!=tensor)
    mask_inf = torch.isinf(tensor)
    if mask_big.any() or mask_nan.any() or mask_inf.any():
        print(f"\nCheck on {tensor_name}:")
        print("Total values:", mask_nan.shape[0])
        print("Number of NaN:", mask_nan.sum().item())
        print("Number of Inf:", mask_inf.sum().item())
        print("Number of very large values:", mask_big.sum().item())
        if mask_big.any():
            print("Very large values:", *list(tensor[mask_big].detach().cpu().numpy()))
    ...
    def forward(self, inputs, cond_inputs=None, mode='direct'):
        if mode == 'direct':
            ...
            if self.pre_exp_tanh:
                a = torch.tanh(a)
                if print_stuff:
                    print_errors(a, "tanh(a)")
            elif self.pre_exp_cut:
                a[a>thresh] = thresh
                a[a<-thresh] = -thresh
...
class BatchNormFlow(nn.Module):
    ...
    def forward(self, inputs, cond_inputs=None, mode='direct'):
        if mode == 'direct':
            ...
            if print_stuff:
                print_errors(mean, "mean")
                print_errors(var, "var")

            x_hat = (inputs - mean) / var.sqrt()

            if print_stuff:
                print_errors(x_hat, "x_hat")
...
class FlowSequential(nn.Sequential):
    ...
    def forward(self, inputs, cond_inputs=None, mode='direct', logdets=None):
        ...
        if mode == 'direct':
            for module in self._modules.values():
                ...
                if print_stuff:
                    if isinstance(module, MADE):
                        count += 1
                        print("\n\n======================")
                        print(f"MADE block {count}")
                        print("======================")
                        print("\n-------------------")
                        print("MADE layer:")
                    elif isinstance(module, BatchNormFlow):
                        print("\n-------------------")
                        print("BatchNorm layer:")
                    elif isinstance(module, Reverse):
                        print("\n-------------------")
                        print("Reverse layer:")
    ...
    def log_probs(self, inputs, cond_inputs = None):
        ...
        if print_stuff:
            print("=============================")
            print("Final output:")
            print_errors(log_probs, "log_probs")
            print_errors(log_jacob, "log_jacob")
